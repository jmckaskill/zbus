...
static int usage(const char *format, ...) {
    ...
    va_end(ap);
    fputs("usage: sockbusd [-v] [-p /path/to/sockbusp] [--] path/to/dbus-socket path/to/sockdir busid\n", stderr);
    return ERR_INVALID_ARG;
}

static int child_process(int sock, const char *sockbusp, char *busdir, char *busid) {
    struct ucred cred;
    ...
    if (pgid < 0) {
        fprintf(stderr, "WARN failed to get process group id for %d: %s\n", cred.pid, strerror(errno));
...
int main(int argc, char *argv[]) {
    ...
    if (verbose) {
        fprintf(stderr, "launching daemon with busdir %s and busid %s\n", busdir, busid);
    ...
    memset(addr.sun.sun_path+pathlen, 0, sizeof(addr.sun.sun_path)-pathlen);
    ...
    if (bind(sock, &addr.sa, offsetof(struct sockaddr_un, sun_path) + pathlen + 1)
    ...
    for (;;) {
        // Set cloexec as we open it. We then reenable after forking so that
        // we don't leak sockets across children.
#ifdef SOCK_CLOEXEC
