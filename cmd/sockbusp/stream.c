...
static const struct msg_header *message_header(struct stream_buffer *b) {
...
int read_message(int fd, struct stream_buffer *b, const struct msg_header **phdr) {
    int sts = do_read(fd, b, ALIGN_UINT_UP(b->off, 8) - b->off + sizeof(struct msg_header));
    if (sts != READ_OK) {
        return sts;
    }
    
    const struct msg_header *h = message_header(b);
...
void drop_message(struct stream_buffer *b) {
    const struct msg_header *h = message_header(b);
    const char *data = message_data(h);
    const char *end = data + h->body_len;
