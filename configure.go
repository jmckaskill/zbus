...
func (s *settings) compile(str string) error {
	...
	args := strings.Split(s.compileFlags, " ")
	log.Printf("CC %q", str)
	cmd := exec.Command(s.compiler, append(args, fn, "-o", f.Name()+".o")...)
...
func (s *settings) gccCCFlags() string {
	return "-c " + s.gccLinkFlags()
}
...
func (s *settings) gccLinkFlags() string {
	if s.enableDebug {
		return "-g -Wall -Werror -I. -o $out -MD -MF $out.d " + s.compileFlags
	} else {
		return "-O2 -DNDEBUG -I. -o $out -MD -MF $out.d " + s.compileFlags
	}
}

func (s *settings) msvcCFlags() string {
	if s.enableDebug {
		return "/nologo /Z7 /MTd /WX /W3 /wd5045 /Od /I. /showIncludes /Fo$out /c " + s.compileFlags
	} else {
		return "/nologo /MT /W3 /DNDEBUG /I. /showIncludes /Fo$out /c " + s.compileFlags
	}
}
...
func (s *settings) writeRules(f *os.File) {
	var cflags, ldflags, libflags, deps string
	switch s.depsType {
	case "gcc":
		deps = " depfile = $out.d\n deps = gcc\n"
		cflags = s.gccCCFlags()
		ldflags = s.gccLinkFlags()
