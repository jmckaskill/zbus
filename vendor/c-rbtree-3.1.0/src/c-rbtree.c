...
static_assert(alignof(CRBTree) <= C_RBTREE_MAX_ALIGN,
	      "Invalid RBTree alignment");
...
static inline void c_rbtree_paint_terminal(CRBNode *n)
{
	...
	if (p == g->left) {
		...
		if (x)
			c_rbnode_set_parent_and_flags(
				x, g, c_rbnode_flags(x) & ~C_RBNODE_RED);
		c_rbnode_set_parent_and_flags(
			p, gg, c_rbnode_flags(p) & ~C_RBNODE_RED);
		c_rbnode_set_parent_and_flags(g, p,
					      c_rbnode_flags(g) | C_RBNODE_RED);
...
static inline CRBNode *c_rbtree_paint_path(CRBNode *n)
{
	...
	for (;;) {
		...
		if (!p) {
			 ...
			 */
			c_rbnode_set_parent_and_flags(n, c_rbnode_raw(n),
						      c_rbnode_flags(n) &
							      ~C_RBNODE_RED);
			return NULL;
		 ...
		 */
		c_rbnode_set_parent_and_flags(
			p, g, c_rbnode_flags(p) & ~C_RBNODE_RED);
		c_rbnode_set_parent_and_flags(
			u, g, c_rbnode_flags(u) & ~C_RBNODE_RED);
		c_rbnode_set_parent_and_flags(g, c_rbnode_raw(g),
					      c_rbnode_flags(g) | C_RBNODE_RED);
		n = g;
	}
}
...
static inline void c_rbnode_rebalance_terminal(CRBNode *p, CRBNode *previous)
{
	...
	if (previous == p->left) {
		...
		if (c_rbnode_is_red(s)) {
			...
			c_rbnode_set_parent_and_flags(
				x, p, c_rbnode_flags(x) & ~C_RBNODE_RED);
			c_rbnode_set_parent_and_flags(
				s, g, c_rbnode_flags(s) & ~C_RBNODE_RED);
			c_rbnode_set_parent_and_flags(
				p, s, c_rbnode_flags(p) | C_RBNODE_RED);
		...
		c_rbnode_swap_child(p, s);
		c_rbnode_set_parent_and_flags(
			x, s, c_rbnode_flags(x) & ~C_RBNODE_RED);
		if (y)
...
static inline CRBNode *c_rbnode_rebalance_path(CRBNode *p, CRBNode **previous)
{
	...
	while (p) {
		 ...
		 */
		if (c_rbnode_is_red(s) || (nl && c_rbnode_is_red(nl)) ||
		 ...
		 */
		c_rbnode_set_parent_and_flags(s, p,
					      c_rbnode_flags(s) | C_RBNODE_RED);
		if (c_rbnode_is_red(p)) {
			c_rbnode_set_parent_and_flags(p, c_rbnode_parent(p),
						      c_rbnode_flags(p) &
							      ~C_RBNODE_RED);
			return NULL;
		}
...
void c_rbnode_unlink_stale(CRBNode *n)
{
	 ...
	 *    Therefore, we can simply swap the node with its successor
	 * (including color) and remove the node from its new place, which will
	 * be one of the first two cases.
	 *
	...
	if (!n->left && !n->right) {
		...
		c_rbnode_set_parent_and_flags(n->right, c_rbnode_parent(n),
					      c_rbnode_flags(n->right) &
						      ~C_RBNODE_RED);
		...
		c_rbnode_set_parent_and_flags(n->left, c_rbnode_parent(n),
					      c_rbnode_flags(n->left) &
						      ~C_RBNODE_RED);
		...
		if (!s->left) {
			...
			c_rbnode_set_parent_and_flags(n->right, s,
						      c_rbnode_flags(n->right));
		}
		...
		c_rbnode_set_parent_and_flags(n->left, s,
					      c_rbnode_flags(n->left));
		...
		if (c)
			c_rbnode_set_parent_and_flags(
				c, p, c_rbnode_flags(c) & ~C_RBNODE_RED);
		else
		...
		if (c_rbnode_is_red(n))
			c_rbnode_set_parent_and_flags(s, c_rbnode_parent(n),
						      c_rbnode_flags(s) |
							      C_RBNODE_RED);
		else
			c_rbnode_set_parent_and_flags(s, c_rbnode_parent(n),
